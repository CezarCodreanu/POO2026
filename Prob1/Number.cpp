...
void Number::fromDecimal(long long value, int newBase) {
    ...
    // Construim cifrele in ordine inversa
    char temp[128];
    int  idx = 0;
    while (value > 0) {
...
Number& Number::operator=(long long value) {
    // Pastreaza baza curenta, schimba doar valoarea
    fromDecimal(value, base);
    return *this;
}

Number& Number::operator=(const char* value) {
    // Pastreaza baza curenta, schimba valoarea (ca string direct)
    delete[] digits;
    length = (int)strlen(value);
    digits = new char[length + 1];
    for (int i = 0; i < length; i++)
...
Number& Number::operator--() {
    // Prefix: sterge primul digit (cel mai semnificativ)
    if (length <= 1) {
        fromDecimal(0, base);
        return *this;
    }
...
Number Number::operator--(int) {
    // Postfix: sterge ultimul digit (cel mai putin semnificativ)
    Number copy = *this;
    if (length > 1) {
        digits[length - 1] = '\0';
        length--;
